...
def comparision_table(results,n):
    p = pd.DataFrame.from_dict(results)
    re_mean = []
    re_std = []
    mean_val = list(p.mean())
    std_val = list(p.std())
    p.insert(0, "First_column", [i for i in range(1,n+1)], True)
    p.loc[len(new_df.index)] = ["mean",mean_val[0],mean_val[1]]
    p.loc[len(new_df.index)] = ["std",std_val[0],std_val[1]]
    return p,mean_val,std_val
